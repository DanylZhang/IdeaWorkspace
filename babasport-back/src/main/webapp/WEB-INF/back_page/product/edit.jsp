            ...
            $("#tabs a").each(function () {
                ...
                $(this).click(function () {
                    ...
                    if (ref == '#tab_2') {
                        ...
                        // fck.Height = 400;
                        // fck.Config['ImageUploadURL'] = '/upload/uploadFck.html';
                        // fck.ReplaceTextarea();
                        var option = {
                            imageActionName: 'test',
                            initialFrameHeight: 500,
                            imageFieldName: 'img',
                            imageUrlPrefix: ''
                        };
                        var ue = UE.getEditor('productdesc', option);
...
<div class="body-box" style="float:right">
    <form id="jvForm" action="/control/product/edit/${product.id}.html" method="post" enctype="multipart/form-data">
        <table cellspacing="1" cellpadding="2" width="100%" border="0" class="pn-ftable">
                    ...
                    <select name="typeId">
                        ...
                        <c:forEach items="${types}" var="type">
                            <option value="${type.id}"
                                    <c:if test="${type.id==product.typeId}">selected="selected"</c:if>>${type.name}</option>
                        </c:forEach>
                    </select>
                </td>
            </tr>
                    ...
                    <select name="brandId">
                        ...
                        <c:forEach items="${brands}" var="brand">
                            <option value="${brand.id}"
                                    <c:if test="${brand.id==product.brandId}">selected="selected"</c:if>>${brand.name}</option>
                        </c:forEach>
                    </select>
                </td>
            </tr>
                ...
                <td width="80%" class="pn-fcontent">
                    <c:forEach items="${features}" var="feature">
                        <input type="checkbox" value="${feature.id}" name="feature"
                               <c:forEach items="${fn:split(product.feature,',')}" var="featureId">
                                   <c:if test="${feature.id==featureId}">checked="checked"</c:if>
                               </c:forEach>
                               />${feature.name}
                    </c:forEach>
                </td>
                ...
                <td width="80%" class="pn-fcontent">
                    <c:forEach items="${colors}" var="color">
                        <input type="checkbox" value="${color.id}" name="color"
                                <c:forEach items="${fn:split(product.color,',')}" var="colorId">
                                    <c:if test="${color.id==colorId}">checked="checked"</c:if>
            ...
            <tr>
                ...
                <td width="80%" class="pn-fcontent">
                    <c:forEach items="${fn:split(\"M,L,XL,XXL\",',' )}" var="size">
                        <input type="checkbox" value="${size}" name="size"
                                <c:forEach items="${fn:split(product.size,',')}" var="psize">
                                    <c:if test="${size==psize}">checked="checked"</c:if>
                                </c:forEach>/>${size}
                    </c:forEach>
            ...
            <tr>
                ...
                <td width="80%" class="pn-fcontent">
                    <input type="checkbox" name="isNew" value="${product.isNew}"/>新品
                    <input type="checkbox" name="isCommend" value="${product.isCommend}"/>推荐
                    <input type="checkbox" name="isHot" value="${product.isHot}"/>热卖
                </td>
            </tr>
            <tr>
                <td width="20%" class="pn-flabel pn-flabel-h">
            ...
            <tr>
                ...
                <td width="80%" class="pn-fcontent">
                    <img width="100" height="100" id="allImgUrl" src="${product.allImgUrl}"/>
                    <input type="hidden" id="path" name="img.url" value="${product.img.url}"/>
                    <input type="file" onchange="uploadPic()" name="pic" value="${product.allImgUrl}"/>
                </td>
            </tr>
            </tbody>
