...
<head>
    ...
    <script type="text/javascript">
        //结算
        function trueBuy() {
            window.location.href = "/buyer/trueBuy.html";
        }
        ...
        //+
        function addProductAmount(skuId, skuUpperLimit) {
            ...
            <div class="ofc pb40">
                    ...
                    <c:forEach items="${buyerCart.buyerItems}" var="item">
                        <tr>
                                    ...
                                    <li>
                                        <a class="pic"
                                           ...
                                           href="javascript:void(0)"><img
                                                ...
                                                src="${item.sku.product.img.allUrl}"></a>
                                        <dl>
                                            <dt>
                                                <a title=" 喜悦2014秋冬新款瑜伽服三件套装 韩版女士瑜珈舞蹈服 愈加服正品送胸垫 酒红+黑+黑 M 开启年终大促 全场优惠到底 买贵就赔 支持货到付款"
                                                   href="javascript:void(0)"> ${item.sku.product.name}--${item.sku.color.name}--${item.sku.size}</a>
                                            </dt>
                            ...
                            <td>
                                ...
                                <input type="text" id="num${item.sku.id}" readonly="readonly" value="${item.amount}"
                                       name=""
                                       size="1" class="txts">
                                <a onclick="addProductAmount(${item.sku.id},${item.sku.skuUpperLimit})" class="inb arr"
                                   title="加"
                                   href="javascript:void(0);">+</a>
                                <dl style="margin-left: 20px;color: red;">
                                    <c:if test="${item.haveInventory}">有货</c:if>
                                    <c:if test="${!item.haveInventory}">无货</c:if>
                                </dl>
                            </td>
				...
				<span class="l">
					...
					<dl class="total">
						...
						<dd><em class="l">运费：</em>￥<var id="deliveFee">${buyerCart.deliveFee}</var>元</dd>
