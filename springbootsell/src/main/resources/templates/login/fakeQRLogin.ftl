...
<script>
    !function () {
        function a(d) {
            jQuery.ajax({
                type: "GET",
                url: "https://long.open.weixin.qq.com/connect/l/qrconnect?uuid=${qrUUID}" + (d ? "&last=" + d : ""),
                dataType: "script",
                cache: !1,
                timeout: 6e4,
                success: function (d, e, f) {
                    ...
                    switch (g) {
                        case 405:
                            break;
                            var h = "http://mp.weixin.qq.com/debug/cgi-bin/sandbox?t=sandbox/login";
                            h = h.replace(/&amp;/g, "&"), h += (h.indexOf("?") > -1 ? "&" : "?") + "code=" + wx_code + "&state=";
                            ...
                            break;
                        case 404:
                            jQuery(".js_status").hide(), jQuery("#wx_after_scan").show(), setTimeout(a, 100, g);
                            break;
                        case 403:
                            jQuery(".js_status").hide(), jQuery("#wx_after_cancel").show(), setTimeout(a, 2e3, g);
                            break;
                        case 402:
                        case 500:
                            break;
        ...
        if (i) {
            ...
            j.rel = "stylesheet", j.href = i.replace(new RegExp("javascript:", "gi"), ""), document.getElementsByTagName("head")[0].appendChild(j)
        }
        setTimeout(a, 100)
    }();
</script>
